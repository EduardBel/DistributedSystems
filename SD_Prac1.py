...
def inicialitza(i):
	...
	"""for a in range(x):
		...
		data=cos.get_object(bucket, 'fila'+str(a+1)+'.txt')
		print(data.decode())
		
	for a in range(z):
		
		data=cos.get_object(bucket, 'col'+str(a+1)+'.txt')
		print(data.decode())"""
...
def funcio_residu_fila(minim):	#minim és la fila on comença a tractar
	...
	while minim<maxim:	#minim és la fila actual a tractar
		...
		dada=dada[:-1]
		dada=dada.encode()
		cos.put_object(bucket, 'fila'+str(int(minim)+1)+'.txt', dada)
...
def funcio_residu_col(minim):	#minim és la fila on comença a tractar
	...
	while minim<maxim:	#minim és la fila actual a tractar
		...
		dada=dada[:-1]
		dada=dada.encode()
		cos.put_object(bucket, 'col'+str(int(minim)+1)+'.txt', dada)
...
def funcio_map(k):
	...
	for a in range(len(k)//int(2)):	
		...
		fila=cos.get_object(bucket, fil)
		columna=cos.get_object(bucket, col)
		fila=fila.decode()
	...
	dada=dada[:-1]	
	dada=dada.encode()	
	cos.put_object(bucket, 'worker'+k[len(k)-1]+'.txt', dada)
	return(k[len(k)-1])
...
def funcio_reduce(results):
	...
	for m in range(len(results)):
			...
			valor=cos.get_object(bucket, 'worker'+results[m]+'.txt')
			valor=valor.decode()
			cont=0
			valor=valor.split(" ")
...
if __name__ == '__main__':
	...
	#sd_map()
	"""data=cos.get_object(bucket, 'map_m1.txt')
	print(data)"""
	...
	"""for i in range(x):					#de les dades que els hi correspon
		...
		cos.put_object(bucket, 'fila'+str(i+1)+'.txt', fila)
		data=cos.get_object(bucket, 'fila'+str(i+1)+'.txt')
	...
	for i in range(z):
		...
		cos.put_object(bucket, 'col'+str(i+1)+'.txt', col)
		data=cos.get_object(bucket, 'col'+str(i+1)+'.txt')
	"""
	...
	#print(cos.get_object(bucket, 'mat_result.txt').decode())
